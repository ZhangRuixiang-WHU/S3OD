...
from mmrotate.models import RotatedBaseDetector, RotatedTwoStageDetector


class MultiSteamDetector(RotatedBaseDetector):
    def __init__(
        self, model: Dict[str, RotatedTwoStageDetector], train_cfg=None, test_cfg=None
    ):
    ...
    def model(self, **kwargs) -> RotatedTwoStageDetector:
        if "submodule" in kwargs:
            assert (
                kwargs["submodule"] in self.submodules
            ), "Detector does not contain submodule {}".format(kwargs["submodule"])
            model: RotatedTwoStageDetector = getattr(self, kwargs["submodule"])
        else:
            model: RotatedTwoStageDetector = getattr(self, self.inference_on)
        return model
