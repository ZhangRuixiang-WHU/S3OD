...
class DeltaXYWHAHBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        if pred_bboxes.size(-1) == 10:
            pred_bboxes = pred_bboxes[...,:5]
        else:
            pass
        if pred_bboxes.size(-1) == 160:
            pred_bboxes_review=pred_bboxes.contiguous().view(pred_bboxes.size(0),-1,10)
            pred_bboxes=(pred_bboxes_review[...,:5]).contiguous().view(pred_bboxes.size(0),-1)
        if self.angle_range in ['oc', 'le135', 'le90']:
    ...
    def decode_sigma(self,
        ...
        if pred_bboxes.size(-1) == 160:
            pred_bboxes_review=pred_bboxes.contiguous().view(pred_bboxes.size(0),-1,10)
            pred_bboxes=(pred_bboxes_review[...,:5]).contiguous().view(pred_bboxes.size(0),-1)
            sigma = (pred_bboxes_review[...,5:]).contiguous().view(pred_bboxes.size(0),-1)
        if self.angle_range in ['oc', 'le135', 'le90']:
            return delta2bbox(bboxes, pred_bboxes, self.means, self.stds,
                              wh_ratio_clip, self.add_ctr_clamp,
                              self.ctr_clamp, self.angle_range,
                              self.norm_factor, self.edge_swap), sigma
        else:
            raise NotImplementedError
