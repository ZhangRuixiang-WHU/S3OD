...
class RBboxMetrics2D(object):
    ...
    def __call__(self,
        ...
        if (ast_bboxes is not None) and (ast_bboxes.numel() != 0):
            return rbbox_metrics(bboxes1.contiguous(), bboxes2.contiguous(), mode,
                                is_aligned, ast_bboxes.contiguous())
        else:
            return rbbox_metrics(bboxes1.contiguous(), bboxes2.contiguous(), mode,
                                is_aligned)
...
def rbbox_metrics(bboxes1, bboxes2, mode='iou', is_aligned=False, ast_bboxes=None):
    ...
    Returns:
        Tensor: shape (m, n) if ``is_aligned`` is False else shape (m,)
    """
    assert mode in ['iou', 'iof','gwd','gwd_10','gjsd','gjsd_10','gjsd_gmm','center_distance2','gaussian_d']
    ...
    if mode == 'gjsd_gmm':
        if ast_bboxes is not None:
            if ast_bboxes.device != bboxes2.device:
                ast_bboxes=ast_bboxes.to(bboxes2)
            g_bboxes1 = xy_wh_r_2_xy_sigma(bboxes1)
            g_bboxes2 = xy_wh_r_2_xy_sigma(bboxes2)
            ast_bboxes = xy_wh_r_2_xy_sigma(ast_bboxes)
            gjsd_1 = get_gjsd(g_bboxes1,g_bboxes2)
            gjsd_2 = get_gjsd(ast_bboxes,g_bboxes2)
            distance_1 = 1/(1+gjsd_1)
            distance_2 = 1/(1+gjsd_2)
            distance = 0.9 * distance_1 + 0.1 * distance_2
        else:
            g_bboxes1 = xy_wh_r_2_xy_sigma(bboxes1)
            g_bboxes2 = xy_wh_r_2_xy_sigma(bboxes2)
            gjsd = get_gjsd(g_bboxes1,g_bboxes2)
    ...
    if mode == 'gaussian_d':
        '''
        get the Gaussian value of a certain feature point to 
        '''

        g_bboxes1 = xy_wh_r_2_xy_sigma(bboxes1)
        g_bboxes2 = xy_wh_r_2_xy_sigma(bboxes2)
        gaussian_d = get_gaussian_value(g_bboxes1,g_bboxes2)
    ...
    if mode == 'gwd_10':
        ...
        g_bboxes2 = xy_wh_r_2_xy_sigma(bboxes2)
        gwd = get_gwd(g_bboxes1,g_bboxes2)
        distance = 1/(2+torch.sqrt(gwd*gwd))
...
def get_gaussian_value(pred, target):
    xy_p, Sigma_p = pred  # mu_1, sigma_1
    xy_t, Sigma_t = target  # mu_2, sigma_2
    ...
    xy_p = xy_p[...,:,None,:2].unsqueeze(-1)
    inv_sigma_t = torch.stack((Sigma_t[..., 1, 1], -Sigma_t[..., 0, 1],
                                -Sigma_t[..., 1, 0], Sigma_t[..., 0, 0]),
                                dim=-1).reshape(-1, 2, 2)
    inv_sigma_t = inv_sigma_t / Sigma_t.det().unsqueeze(-1).unsqueeze(-1)
    gaussian = torch.exp(-0.5*(xy_p-xy_t).permute(0,1,3,2).matmul(inv_sigma_t).matmul(xy_p-xy_t)).squeeze(-1).squeeze(-1)/(2*3.1415926*Sigma_t.det())
