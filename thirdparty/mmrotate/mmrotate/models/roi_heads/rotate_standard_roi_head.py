...
class RotatedStandardRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def simple_test_bboxes(self,
        ...
        if rois.shape[0] == 0:
            ...
            if return_ass_cent:
                return [det_bbox] * batch_size, [det_label] * batch_size,  [ast_ct] * batch_size
            else:
            ...
            if rois[i].shape[0] == 0:
                ...
                if rcnn_test_cfg is None:
                    ...
                    det_label = rois[i].new_zeros(
                        (0, self.bbox_head.fc_cls.out_features))
                assist_center = det_bbox[:,:2]
                assist_centeres.append(assist_center)
            
            elif return_ass_cent:
                det_bbox, det_label, assist_center = self.bbox_head.get_bboxes(
                    rois[i],
                    cls_score[i],
                    bbox_pred[i],
                    img_shapes[i],
                    ...
                    rescale=rescale,
                    cfg=rcnn_test_cfg,
                    return_ass_cent=return_ass_cent)
        ...
        if return_ass_cent:
            return det_bboxes, det_labels, assist_centeres
        else:
