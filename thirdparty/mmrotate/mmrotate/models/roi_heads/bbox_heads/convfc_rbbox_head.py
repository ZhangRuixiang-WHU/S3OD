...
class RotatedConvFCBBoxHead(RotatedBBoxHead):
    ...
    def __init__(self,
        ...
        if self.use_gaussian:
            ...
            reg_num=5
        if self.with_reg:
            out_dim_reg = (reg_num if self.reg_class_agnostic else reg_num *
                           self.num_classes)
...
@ROTATED_HEADS.register_module()
class RotatedConvFCBBoxHeadReweight(RotatedBBoxHeadReweight):
    r"""More general bbox head, with shared conv and fc layers and two optional
    separated branches.
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(RotatedConvFCBBoxHeadReweight, self).__init__(
            *args, init_cfg=init_cfg, **kwargs)
        assert (num_shared_convs + num_shared_fcs + num_cls_convs +
                num_cls_fcs + num_reg_convs + num_reg_fcs > 0)
        if num_cls_convs > 0 or num_reg_convs > 0:
...
@ROTATED_HEADS.register_module()
class RotatedShared2FCBBoxHeadReweight(RotatedConvFCBBoxHeadReweight):
    """Shared2FC RBBox head."""

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(RotatedShared2FCBBoxHeadReweight, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
