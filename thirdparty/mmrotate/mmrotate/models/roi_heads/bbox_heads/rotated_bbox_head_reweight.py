...
from mmrotate.core import build_bbox_coder, multiclass_nms_rotated, multiclass_nms_rotated_sigma
from ...builder import ROTATED_HEADS, build_loss
import numpy as np


@ROTATED_HEADS.register_module()
class RotatedBBoxHeadReweight(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(RotatedBBoxHeadReweight, self).__init__(init_cfg)
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        ratio_pos_neg = np.clip(3*num_pos/num_neg,0.2,1.0)
        ...
        pos_area = (pos_bboxes[:,2]-pos_bboxes[:,0]) * (pos_bboxes[:,3]-pos_bboxes[:,1]) 
        pos_small_id = pos_area <= 600
        pos_large_id = pos_area > 600
        if num_pos > 0:
            weight_small = 1.0
            if pos_large_id.sum() > 0:
                weight_small = torch.clamp((pos_large_id.sum())/(pos_small_id.sum()),1.0,5.0)
            labels[:num_pos] = pos_gt_labels
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight
            label_weights[:num_pos] = pos_weight
            (label_weights[:num_pos])[pos_small_id] = weight_small
            label_weights
            ...
            (bbox_weights[:num_pos, :])[pos_small_id] = weight_small
            bbox_weights = (num_pos*4/bbox_weights.sum().item())* bbox_weights ## for normalization
        if num_neg > 0:
            label_weights[-num_neg:] = ratio_pos_neg#1.0
        
        label_weights = (num_samples/label_weights.sum().item())* label_weights ## for normalization
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    pos_bbox_pred = bbox_pred.view(
                        bbox_pred.size(0), self.reg_num)[pos_inds.type(torch.bool)]
                else:
                    pos_bbox_pred = bbox_pred.view(
                        bbox_pred.size(0), -1,
                        self.reg_num)[pos_inds.type(torch.bool),
    ...
    def get_bboxes(self,
        ...
        if bbox_pred is not None:
            if return_sigma:
                bboxes,sigma = self.bbox_coder.decode_sigma(
        ...
        elif return_ass_cent:
            det_bboxes, det_labels, assist_center = multiclass_nms_rotated(
                bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img, return_ass_cent=True)
            return det_bboxes, det_labels, assist_center
        elif return_sigma:
            det_bboxes, det_labels = multiclass_nms_rotated_sigma(
                bboxes, sigma, scores, cfg.score_thr, cfg.nms, cfg.max_per_img)
            # # sigma_mean = bbox_pred[...,5:].sum(dim=-1) * 0.2
            # # sigma = 1- sigma_mean[keep]
            # sigma_mean = bbox_pred[...,5:].sum(dim=-1) * 0.2
            # sigma = torch.sigmoid(sigma_mean[keep])
            # sigma = (torch.ones_like(sigma)-sigma).view(-1,1)
            # # sigma = (1- sigma).view(-1,1)
            # det_bboxes = torch.cat([det_bboxes,sigma],dim=-1)
            return det_bboxes, det_labels#, sigma
        else:
