...
class RandomSamplerNeg(BaseSampler):
    """Random sampler.
    ...
    def __init__(self,
        ...
        super(RandomSamplerNeg, self).__init__(num, pos_fraction, neg_pos_ub,
                                            add_gt_as_proposals)
    ...
    def sample(self,
        ...
        if gt_neg_box is not None:
            ...
            if len(gt_neg_box) > 0:
                gt_neg_labels = -bboxes.new_zeros(gt_neg_box.shape[0], dtype=torch.uint8)
                bboxes = torch.cat([bboxes, gt_neg_box], dim=0)
                assign_result.add_gt_neg_(gt_neg_labels)
                gt_neg = bboxes.new_zeros(gt_neg_box.shape[0], dtype=torch.uint8)
                gt_flags = torch.cat([gt_flags, gt_neg])
