...
class MaxIoUAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None,cls_scores=None):
        """Assign gt to bboxes.
        ...
        if self.assign_metric is not None:
        ...
        # #     samll_sizes = gt_sizes <= 1024
        # #     small_gt_id = torch.where(samll_sizes==True)[0]
        # #     large_gt_id = torch.where(samll_sizes==False)[0]
        # #     small_gt_id = small_gt_id.add(1)
        # #     large_gt_id = large_gt_id.add(1)
        # #     small_num = small_gt_id.size(0)
        # #     large_num = large_gt_id.size(0)
        # #     assert (small_num + large_num) == gt_sizes.size(0)
        # #     anchor_s = 0
        # #     anchor_l = 0
        # #     for s_idx in small_gt_id:
        # #         anchor_s += (torch.where(assign_result.gt_inds==s_idx)[0]).size(0)
        # #     for l_idx in large_gt_id:
        # #         anchor_l += (torch.where(assign_result.gt_inds==l_idx)[0]).size(0)
        # #     if debug:
        # #         FilePath = "/home/zrx/ssod/SoftTeacher/work_dirs/s3od_r1_nosla_percent.txt"
        # #         with open(FilePath, "a") as filewrite:   #”w"代表着每次运行都覆盖txt的内容
        # #             filewrite.write(str(large_num) + " " + str(anchor_l) + " " + str(small_num) + " " + str(anchor_s) +"\n")
        # if gt_labels is None: # means the assign process is in RPN stage, rather than R-CNN stage
        #     if gt_bboxes.size(1) == 4:
        #         gt_sizes = (gt_bboxes[:,2]-gt_bboxes[:,0]) * (gt_bboxes[:,3]-gt_bboxes[:,1])
        #     else:
        #         gt_sizes = gt_bboxes[:,2] * gt_bboxes[:,3]
